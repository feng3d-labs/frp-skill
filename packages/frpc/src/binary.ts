...
// 缓存目录，用于存储下载的二进制文件
const CACHE_DIR = path.join(os.tmpdir(), '@feng3d-frpc');
...
export function getPlatformInfo(): PlatformInfo {
  ...
  const envPlatform = process.env.FRPC_PLATFORM || process.env.PLATFORM;
...
async function downloadFromGitHub(platform: NodeJS.Platform, arch: string): Promise<string> {
  ...
  // 下载到临时目录
  const tempDownloadDir = path.join(os.tmpdir(), `frpc-download-${Date.now()}`);
  await fs.mkdir(tempDownloadDir, { recursive: true });

  const tarballPath = path.join(tempDownloadDir, `frpc.${ext}`);
...
export async function getBinaryPath(platform?: NodeJS.Platform, arch?: string): Promise<string> {
  ...
  const binaryName = targetPlatform === 'win32' ? 'frpc.exe' : 'frpc';
  ...
  try {
    ...
    throw new Error(
      `无法获取 frpc 二进制文件 (${targetPlatform} ${targetArch})\n` +
