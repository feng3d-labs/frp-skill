...
export async function getLocalBinary(platform: NodeJS.Platform, arch: string): Promise<string | null> {
  ...
  try {
    ...
    const localPlatformPkg = path.join(workspaceRoot, 'packages', platformPackageName.replace('@feng3d/frpc-', 'frpc-'), binaryName);
    try {
...
export async function downloadFrpc(options: DownloadOptions): Promise<string> {
  const {
    version,
...
export async function extractZip(zipPath: string, destDir: string): Promise<string> {
  ...
  // Windows 下使用 PowerShell 解压
  const command = `Expand-Archive -Path "${zipPath}" -DestinationPath "${destDir}" -Force`;
  await execaCommand(`powershell -Command "${command}"`, { stdio: 'inherit' });
